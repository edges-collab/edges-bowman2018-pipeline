...
@click.option("--output-dir", type=click.Path(exists=True, dir_okay=True, file_okay=False), default='output-notebooks')
...
@click.pass_context
def run(ctx, kernel, formats, ipynb, output, output_dir, convert_args, toml, days, threads):
    ...
    ctx.obj['toml'] = toml
    ctx.obj['days'] = sorted(set(sum([list(range(*tuple(int(n) for n in d.split("~")))) for d in days], start=[])))
...
def run_notebook_factory(notebook):
    ...
    params = [
        ...
        for param, v in infer.items() if v['inferred_type_name'] in tps
    ]
